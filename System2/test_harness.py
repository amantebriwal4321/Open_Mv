"""
test_harness.py — Accuracy testing for System 2.
...
# Add parent folder (System2) to path first to ensure we import System 2 modules
...
def run_test(folder, report_path="test_report.csv"):
    ...
    print(f"[TEST - SYSTEM 2] Processing {len(files)} images from {folder}\n")
    print(f"{'File':<35} {'Label':<7} {'A1':<10} {'A2 (CS)':<10} {'A3':<10} {'Ens':<10} {'Conf':>6} {'ms':>6}")
    print("-" * 95)
    ...
    if labelled > 0:
        ...
        for key in ["approach1", "approach2", "approach3", "ensemble"]:
            ...
            name = {"approach1": "Colour", "approach2": "Centroid Shift",
                    "approach3": "Density", "ensemble": "ENSEMBLE"}[key]
            print(f"  {name:<16}: {c}/{n} = {acc:.1f}%")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test System 2 chilli detection accuracy")
