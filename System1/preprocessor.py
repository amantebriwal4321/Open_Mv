"""
preprocessor.py — Preprocessing pipeline for the chilli detection system.
...
def enhance_contrast(frame):
    ...
    Improves detection under shadows in the V-channel.
...
def mask_reflections(frame):
    ...
    Pixels above REFLECTION_THRESHOLD in all channels
    are set to the local median to reduce glare.

    Args:
        frame: BGR image.

    Returns:
        Frame with reflections suppressed, and the reflection mask.
    """
...
def crop_roi(frame):
    ...
    Uses fractional ROI coordinates from config so it
    adapts to any resolution.

    Args:
        frame: BGR image (full camera frame).

    Returns:
        Cropped BGR image, and (x_offset, y_offset) tuple
        so we can map coordinates back to the full frame.
    """
    h, w = frame.shape[:2]
...
def detect_chilli_present(hsv, min_area=None):
    ...
    Args:
        hsv: HSV image.
        min_area: Override for minimum red pixel count.

    Returns:
        (is_present: bool, red_mask: numpy array)
    """
    if min_area is None:
...
def preprocess(frame):
    ...
    Returns:
        dict with keys:
            'processed'     : cleaned BGR image (ROI cropped)
            'hsv'           : HSV conversion of processed
            'red_mask'      : binary mask of red pixels
            'chilli_present': bool
            'roi_offset'    : (x, y) offset for mapping back
            'reflection_mask': binary mask of reflections
        Returns None if frame is empty.
    """
