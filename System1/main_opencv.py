"""
main_opencv.py — Chilli Stem Detection System (OpenCV Version)

Main loop with:
  - Live camera feed + trackbar controls
  - All 3 detection approaches with priority execution
  - Ensemble voting
  - 4-window visualisation
  - CSV logging + uncertain image saving
  - Simulated IO signal output

Usage:
    python main_opencv.py              # live camera
    python main_opencv.py --image X    # single image
    python main_opencv.py --folder X   # batch folder
"""
...
def log_result(data):
    ...
    Args:
        data: dict with all result fields.
    """
...
def save_uncertain_image(frame, confidence, stem_side):
    ...
    Args:
        ...
        stem_side: detected side string.

    Returns:
        Filename of saved image or empty string.
    """
...
def send_io_signal(stem_side, confidence):
    ...
    On laptop: print to console.
    On OpenMV: would toggle pins.

    Args:
        stem_side: 'LEFT', 'RIGHT', or 'UNCERTAIN'.
    ...
    if stem_side == "LEFT" and confidence >= cfg.ENSEMBLE_MED_CONF:
        print(f"  [IO] Pin1=HIGH  Pin2=LOW   | STEM LEFT  | "
              f"Conf={confidence:.1f}% [{conf_category}]")
        return True
    elif stem_side == "RIGHT" and confidence >= cfg.ENSEMBLE_MED_CONF:
        print(f"  [IO] Pin1=LOW   Pin2=HIGH  | STEM RIGHT | "
              f"Conf={confidence:.1f}% [{conf_category}]")
        return True
    else:
        print(f"  [IO] Pin1=LOW   Pin2=LOW   | UNCERTAIN  | "
              f"Conf={confidence:.1f}% [{conf_category}]")
        return False
...
def draw_visualisation(frame, results, processing_ms):
    ...
    Args:
        ...
        results: dict containing all detection outputs.
        processing_ms: float, processing time.
    ...
    approach_names = {0: "Colour", 1: "Density", 2: "Skeleton", -1: "None"}

    # Bounding box from approach 2
    a2 = results.get("approach2", {})
    bbox = a2.get("bbox")
    if bbox:
        x, y, w, h = bbox
        cv2.rectangle(display, (x, y), (x + w, y + h), (0, 255, 255), 2)

    # Stem marker (red circle)
...
def build_mask_view(red_mask, stem_mask):
    """Combine red and stem masks into a colour visualisation.

    Args:
        red_mask: Binary mask of chilli body.
        stem_mask: Binary mask of stem (green+yellow).

    Returns:
        BGR image showing red mask in red, stem mask in green.
    """
...
def build_skeleton_view(gray, skeleton_result):
    """Overlay skeleton, junctions and endpoints on grayscale.

    Args:
        gray: Grayscale image.
        skeleton_result: dict from approach_skeleton().
    ...
    for jx, jy in skeleton_result.get("junctions", []):
        cv2.circle(vis, (jx, jy), 4, (0, 255, 255), -1)  # yellow

    for ex, ey in skeleton_result.get("endpoints", []):
        cv2.circle(vis, (ex, ey), 4, (0, 255, 0), -1)     # green

    stem_pt = skeleton_result.get("stem_point")
    if stem_pt:
        cv2.circle(vis, stem_pt, 8, (0, 0, 255), 2)
...
class StatsTracker:
    ...
    def draw_dashboard(self, width=400, height=400):
        ...
        cv2.putText(dash, "=== STATS DASHBOARD ===", (10, y), font, 0.6, (0, 200, 255), 1)
        y += 35
        ...
        y += 25
        names = {0: "Colour", 1: "Density", 2: "Skeleton"}
        for i in range(3):
            pct = (self.approach_usage.get(i, 0) / max(self.total, 1)) * 100
            bar_len = int(pct * 1.5)
            cv2.rectangle(dash, (120, y - 12), (120 + bar_len, y + 2), (0, 180, 0), -1)
...
def run_detection(frame):
    """Execute the full detection pipeline on one frame.

    Priority execution:
        1. Approach 1 (colour) — if confidence >= 75%, done.
        2. Approach 2 (density) — if confidence >= 70%, done.
        3. Approach 3 (skeleton) — always returns a guess.
        4. Ensemble vote across all that ran.

    Args:
        frame: Raw BGR frame from camera.

    Returns:
        dict with all results, or None if no chilli present.
    """
    ...
    # --- Approach 3: Skeleton (run always for ensemble) ---
...
def main():
    parser = argparse.ArgumentParser(
        description="Chilli Stem Detection System"
    )
    ...
    create_trackbars()
    print("[INFO] System running. Press 'q' to quit, 's' to save frame.")
    ...
    while True:
        ...
        if not results.get("chilli_present"):
            # Show raw feed with "No Chilli" label
        ...
        # ── Window 3: Skeleton debug ──
        gray = cv2.cvtColor(processed, cv2.COLOR_BGR2GRAY)
        skel_view = build_skeleton_view(gray, results["approach3"])
        cv2.imshow("Skeleton", skel_view)
