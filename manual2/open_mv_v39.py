...
# ==================================================================================
#  VERSION 39     (bumped every time the code changes - check this first)
# ----------------------------------------------------------------------------------
...
def draw_scene(img, r, headline, color, sub, fps):
    ...
    if r["reason"] == "ok" and r["lo"] >= 0:
        ...
        # a deep block over the stopper end of the chute, not a thin outline
        mk = span_rect(0, 4)
        draw_rect(img, mk, col, 4)

        # a filled tag beside it carrying the word, at double size
        cx, cy, cw, ch = CHANNEL_ROI
        tw, th = 4 * 12 + 12, 24
        tx = cx - tw - 6
        if tx < 0:
        ...
        draw_str(img, tx + 6, ty + 5, label, color=col, scale=2)
