...
# ==================================================================================
#  VERSION 21     (bumped every time the code changes - check this first)
# ----------------------------------------------------------------------------------
...
W_STALK       = 1.6      # stalk on that end     (only counted when one is seen)
...
def reset_reference():
    global ref, ref_frames, ref_t, lim_auto
    ref = [1.0] * BANDS
    ref_t = [1.0] * BANDS
    ref_frames = 0
    lim_auto = None
...
def loose_floor(img, lo, hi, lim2):
    """The empty-channel reading at the looser brightness limit.

    Measured at a band that is currently outside the pod, so it carries the
    rails and any fixed darkness but no chilli.
    """
    cand = []
    if lo - 2 >= 0:
        cand.append(0)
...
def look(img):
    ...
    if ref_frames < REF_WARMUP and MANUAL_L is None:
        learn_threshold(img)
    ...
    fidx = [i for i in range(lo, hi + 1) if flesh[i] >= BAND_ON]
    ...
    stalk_near = (lo_f - lo) + stalk_run(img, prof, lo - 1, -1, lim2, floor2)
    stalk_far = (hi - hi_f) + stalk_run(img, prof, hi + 1, +1, lim2, floor2)
