...
def check_stalk(img, E_stop, E_far, lim):
    ...
    roi_near = clamp_roi(nx - half, ny - half, STALK_BOX, STALK_BOX)

    fx = int(E_far[0] + STALK_REACH * ux)
    fy = int(E_far[1] + STALK_REACH * uy)
    roi_far = clamp_roi(fx - half, fy - half, STALK_BOX, STALK_BOX)
    ...
    if d_near is None or d_far is None:
        return 0.0                         # a box fell off the frame: no opinion
