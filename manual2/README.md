...
|---|---|---|
| `open_mv_v37.py` | 37 | **Current.** Big Detecting Box: Expanded `CHANNEL_ROI = (192, 30, 48, 182)` (width 48px, height 182px) for generous coverage of entire chute without clipping curved chillies, larger UI indicator, and color-lightness stalk validator (`STALK_MAX_RED = 8.5`). |
| `open_mv_v36.py` | 36 | Fixed desk mat bleed above chute: calibrated `CHANNEL_ROI = (205, 48, 26, 145)`. |
| `open_mv_v35.py` | 35 | Fixed apex tip misidentified as stalk: added Stalk-Shoulder Consistency Rule. |
| `open_mv_v33.py` | 33 | 24-Band Width Profile with per-band background learning and triple-threshold stalk checking. |
| `open_mv_v28.py` | 28 | UI only: marker box pinned to stopper (band 0) displaying green STEM / red APEX. |
| `open_mv_v27.py` | 27 | **A chilli is one connected object.** The flesh extent used to be first-to-last matching band, gaps and all - on the machine, flesh at bands 19-20, nothing at 21, then background dark enough to pass the flesh limit at 22-23 - so a detached patch was pulled into the flesh and taper's far third averaged it with real chilli. In the offline reconstruction v26 comes out at `APEX -0.11, taper -0.08`, exactly the weak wrong answer the machine used to give; v27 lands `STEM +0.47`. The flesh extent is now the contiguous run **nearest the stopper** that is long enough to be a pod; anything detached beyond it is something else in frame. Same reasoning as v26's stalk rule, applied to the flesh. |
| `open_mv_v26.py` | 26 | **A stalk has to end.** v25 flagged `POD RUNS OFF FAR END` correctly and then used the corrupted measurement anyway: the grey background above the chute filled the far bands (`any` 0.33-0.49, flesh 0.00), was counted as a five-band stalk, and overrode a **correct** taper of +0.45 at weight 2.4. The flesh cues were unharmed — mid-grey never passes the flesh limit — so only the stalk cue was poisoned. A stalk is now a *contiguous* run outward from the flesh, and a far-end run that reaches the edge of the channel is rejected: a stalk stops, background does not. The near end keeps the opposite rule, since a stalk there *should* reach the edge with the pod against the stopper. |
| `open_mv_v25.py` | 25 | Flags **`POD RUNS OFF FAR END`**. A real chilli ends before the box does; when the object reaches the last band, either `CHANNEL_ROI` is longer than the chute and its far end is over something dark, or the pod is longer than the box. Either way the far third is not chilli and taper compares against a fiction — it read `score -0.09, taper -0.07` and quietly said APEX. Also: **every decision line now carries the version number**, because two rounds were spent on output from a version that was not the one being discussed. |
| `open_mv_v24.py` | 24 | The empty-chute reference now learns **only from an empty chute**, and its drift is measured **per second instead of per frame**. It used to update on every frame, including while a pod sat in front of it, so it slowly absorbed the pod — worst in the bands where pods habitually rest, i.e. the stopper end. The flesh there read thin, taper went negative, and the answer decayed to APEX on pod after pod. At the camera's real 215 fps the per-frame drift climbed 0.1 every nine seconds; in the new 60-second dwell test the old code loses the pod **entirely** (`bands -1--1`). Also: the stalk cue now requires a real signal (`STALK_MIN_W`) instead of counting 1-2 pixels of noise, the loose limit is placed proportionally between body and chute rather than a fixed margin below an overexposed white, the profile dump prints the `any` column that drives extent and stalk, and the blue marker is labelled `STEM?`. |
| `open_mv_v23.py` | 23 | The stalk limit is now set by the **chute**, not by the body limit. It was `lim + 20` = 69, while a dried stalk on a white chute sits around L 70-75 — the threshold meant to find the stalk was *brighter than the stalk*, so it was never measured and the cue read `+0`. The empty-chute brightness is learned during warm-up (as a high percentile, so dark rails and the stopper bar cannot drag it down) and the limit is `chute - CHUTE_MARGIN`. Also: the banner is placed beside `CHANNEL_ROI` instead of on top of it, and the band-by-band profile prints on **every locked decision**, not only in CALIBRATE. |
