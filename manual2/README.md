...
|---|---|---|
| `open_mv_v28.py` | 28 | **Current.** UI only: the marker box no longer jumps between the two ends. It stays **pinned on the stopper end** (band 0) and just changes label and colour — green **STEM** if a stem is at the stopper, red **APEX** if an apex is. Same detection logic; only the display changed. The moving box read as "the code is looking at the wrong side", which it never was. |
