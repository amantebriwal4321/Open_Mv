...
# ==================================================================================
#  VERSION 18     (bumped every time the code changes - check this first)
# ----------------------------------------------------------------------------------
...
W_RED         = 0.3      # the stem end is the paler end   (only counted when read)
A_FULL        = 12.0
...
def stalk_run(img, prof, start, step, lim2):
    ...
    while 0 <= i < BANDS and n <= MAX_STALK_BANDS:
        ...
        if dark_fraction(img, band_roi(i), lim2) < BAND_THIN:
...
def look(img):
    ...
    prof = [dark_fraction(img, band_roi(i), lim) for i in range(BANDS)]
    out["prof"] = prof
    ...
    stalk_near = stalk_run(img, prof, lo - 1, -1, lim2)
    stalk_far = stalk_run(img, prof, hi + 1, +1, lim2)
...
while True:
        ...
        if present:
            ...
            print("CALIB score=%+.2f -> %-4s | taper %+.2f (%.2f vs %.2f) | "
                  ...
                  % (avg, NAME[live], r["s_taper"], r["w_near"], r["w_far"],
                     ...
                     r["s_centroid"], r["s_red"], r["lo"], r["hi"], r["red"],
                     "" if r["agree"] else "  CUES DISAGREE"))
        else:
            txt = REASON_TEXT.get(r["reason"], "EMPTY")
            draw_scene(img, r, txt, (255, 255, 255), "no reading", fps)
            print("CALIB %-20s bands %d-%d  a=%.1f  L<=%d"
                  % (txt, r["lo"], r["hi"], r["red"], r["lim"]))
        continue
    ...
    if state in (LOCKED, CLEARING) and final:
        ...
        txt = REASON_TEXT.get(r["reason"], "EMPTY")
        draw_scene(img, r, txt, (255, 255, 255), "waiting | #%d" % total, fps)
        service_leds(None)
