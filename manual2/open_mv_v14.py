...
#      Automatic channel finding (v13) grabbed too much of the picture and
#      cost speed, so AUTO_CHANNEL is off.
#      Smoother: the score is now averaged over 9 frames instead of 5, and an
#      answer must hold for 5 frames instead of 3 before it locks.
# ----------------------------------------------------------------------------------
...
# the grey mat: the mat is darker than the bright channel, so the camera would
# lock onto the mat and ignore the chili lying beside it.
#   box too far LEFT  -> raise the first number (186 -> 196)
...
DECIDE_MIN     = 0.15
STABLE_N       = 5               # frames the answer must hold before it locks
CLEAR_FRAMES   = 4               # empty frames before the next chili
MAX_WAIT_MS    = 1500
VOTE_HISTORY_MAX = 9             # frames averaged - higher = smoother
