...
# ==================================================================================
#  VERSION 42     (bumped every time the code changes - check this first)
# ----------------------------------------------------------------------------------
...
TIGHT_MIN     = 12
STALK_SPAN    = 0.72
# ...and never leave less than this much room above the chilli limit, whatever
# the chute measures. This is the floor that stops the stalk going missing.
STALK_MIN_GAP = 20
STALK_L_CAP   = 90
STALK_MIN_W   = 0.08
...
# How long without a single empty frame before the reference is SUSPECTED of
# being wrong. It is only ever a warning now - see the note where it is used.
REF_STALE_MS   = 120000
...
# A high percentile, not a mean or a low one. At 0.75 this read 61 on a chute
# that is plainly white, because a quarter of the ROI is the darker wall beside
# it plus whatever pod is lying in it. Every limit below is derived from this
# number, so under-reading it squeezes the whole scale.
CHUTE_PCTL = 0.90
...
def object_limit(lim):
    ...
    STALK_MIN_GAP is the floor, and it matters more than it looks. On the
    machine this came out at 57 against a chilli limit of 47: ten levels of
    room, while a dried stalk sits at 60-70. The stalk was therefore not merely
    weighted low, it was NOT MEASURED - the profile printed "no stalk seen" on a
    frame where the stalk is the most obvious thing in the picture, and taper
    was left to decide alone with the stalk counted as a thin end, i.e. an apex.
    No weighting or colour test can rescue a cue that never sees anything.
    """
    if chute_L is None:
        return STALK_L_CAP
    span = max(0.0, chute_L - lim)
    return int(min(STALK_L_CAP, max(lim + STALK_MIN_GAP,
                                    lim + span * STALK_SPAN)))
...
def look(img):
    ...
    # A long spell with no empty frame USED TO force the reference to re-learn
    # from whatever was in front of the camera. That was meant to recover from a
    # bad reference; what it actually did was absorb a chilli that had simply
    # been left sitting at the stopper for a minute, after which the pod was
    # part of the background and the screen read EMPTY with a chilli plainly in
    # the chute. A watchdog that quietly destroys good data is worse than no
    # watchdog. It now only says so, and lets the operator clear the chute.
    global stale_warned
    ...
    if stale and not stale_warned:
        stale_warned = True
        print("!!! no empty chute seen for %ds. If the screen is wrong about"
              % (REF_STALE_MS // 1000))
        print("    what is in the chute, clear it for a moment so the")
        print("    background can be re-learned.")
    if not stale:
        stale_warned = False

    if warming or seems_empty:
...
def draw_scene(img, r, headline, color, sub, fps, answer=None):
    if not DRAW_OVERLAY:
        return
    if DEBUG:
    ...
    # Pin the answer to the stopper end (band 0), drawn big enough to read at a
    # glance from across the machine rather than by squinting at the frame.
    if r["reason"] == "ok" and r["lo"] >= 0:
        # The LOCKED answer when there is one, not this frame's raw score. The
        # two are different things - the lock is a smoothed decision held over
        # several frames - and drawing the raw one put a red APEX tag beside a
        # banner reading STEM.
        is_stem = (answer == "STEM") if answer else (r["score"] > 0)
        col = (0, 220, 0) if is_stem else (255, 60, 60)
        label = "STEM" if is_stem else "APEX"

        # A deep block over the stopper end of the chute. This is the answer,
        # so it is drawn to be read from across the machine, not squinted at.
        mk = span_rect(0, 6)
        draw_rect(img, mk, col, 5)

        # ...and a filled tag beside it carrying the word at triple size.
        SC = 3
        cx, cy, cw, ch = CHANNEL_ROI
        tw, th = 4 * 6 * SC + 14, 7 * SC + 14
        tx = cx - tw - 6
        if tx < 0:
            tx = min(319 - tw, cx + cw + 6)
        ty = min(240 - th - 1, max(0, mk[1] + (mk[3] - th) // 2))
        draw_rect(img, (tx, ty, tw, th), (0, 0, 0), 1, True)
        draw_rect(img, (tx, ty, tw, th), col, 3)
        draw_str(img, tx + 7, ty + 7, label, color=col, scale=SC)
        ...
        if not present:
            ...
            if hold_n >= need or timed_out:
                ...
                if hold_n >= need:
                    ...
                    # Timed out. hold_answer is whatever the score last said
                    # while it was above DECIDE_MIN, and by now the score may
                    # have drifted the other way - which is how the screen came
                    # to read "STEM (P0)" beside a score of -0.11, with the
                    # marker saying APEX. Read the answer off the score being
