...
# ==================================================================================
#  VERSION 20     (bumped every time the code changes - check this first)
# ----------------------------------------------------------------------------------
...
BLANK = {"reason": "empty", "score": 0.0, "lim": 0, "lo": -1, "hi": -1,
         ...
         "floor": 0.0, "wrong_end": False, "raw": None}
...
def print_profile(r):
    ...
    for i in range(BANDS):
        ...
        if i == r["hi"]:
            tag += "  [pod ends]"
        print("   %2d  | %.2f %.2f = %.2f | %-20s%s"
              % (i, r["raw"][i], ref[i], v, "#" * int(v * 20 + 0.5), tag))
    print("  near third %.2f   far third %.2f   taper %+.2f -> %s"
