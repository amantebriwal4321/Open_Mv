...
# ==================================================================================
#  VERSION 32     (bumped every time the code changes - check this first)
# ----------------------------------------------------------------------------------
#  v28  UI: PIN THE MARKER TO THE STOPPER. Same answer, better shown.
#
#  The blue "STEM?" marker used to jump to whichever end won the vote. When the
#  code decided APEX it moved to the far end, which read as "the code is looking
#  at the wrong side". It never was - the question being answered has always
#  been "what is at the stopper" - but the display made it look otherwise.
#
#  The marker now stays fixed on the stopper end (band 0). Only the label
#  changes: green STEM when a stem is at the stopper, red APEX when an apex is.
...
FIT_ROI       = False    # True = with the chute EMPTY, measure where the bright
                         # chute actually is and print the CHANNEL_ROI to use.
                         # Runs once at start-up, then carries on as normal.
                         # Beats reading coordinates off a screenshot, which is
                         # guesswork and has been wrong more than once.
DRAW_OVERLAY  = True     # False = draw NOTHING on the picture. Set this before
                         # saving a frame to analyse later: the boxes and text
                         # are drawn INTO the image, so a saved frame with them
                         # on feeds the code's own graphics back in as pixels.

# The metal chute, as a box: (x, y, width, height).
# Must sit ON THE METAL ONLY and must stop at the stopper bar.
CHANNEL_ROI   = (146, 50, 28, 138)
#                x    y   w    h
...
def stalk_run(loose, start, step):
    ...
    """
    # One thin band is allowed to dip below the limit without ending the run.
    # A stalk is only a few pixels wide, so its measured width wobbles from band
    # to band; on a real frame bands reading 0.22, 0.10, 0.24, 0.09 were thrown
    # away entirely because the next one read 0.04 against a limit of 0.05.
    # Demanding an unbroken run asks for more steadiness than a 3-pixel stalk
    # can give.
    n, i, gap = 0, start, 0
    last_good = start
    while 0 <= i < BANDS and n <= MAX_STALK_BANDS + 1:
        if loose[i] >= BAND_THIN:
            n += gap + 1              # the skipped band is part of the stalk
            gap = 0
            last_good = i
        else:
            gap += 1
            if gap > 1:
                break
        i += step
    # ran_off only counts if the run really walked out of the channel
...
def look(img):
    ...
    #
    # But note what this costs when CHANNEL_ROI is too SHORT for the pod. A real
    # stalk at the far end then runs off the edge too, and gets thrown away with
    # the background - so the single best cue is discarded on exactly the pods
    # it would have answered. That is not a bug in the rule, it is the rule
    # being asked a question it cannot answer: if the pod leaves the box, what
    # is beyond the edge is unknowable. Say so instead of proceeding as if the
    # cue had simply found nothing.
    if ran_off_far:
...
def longest_run(flags):
    """(start, length) of the longest True run, or (-1, 0)."""
...
def print_profile(r):
    ...
    # These interpretation lines MUST go through the same inversion the final
    # answer does, or they contradict it. With INVERT_ANSWER on they read the
    # raw cue sign and printed "taper -> STEM at stopper" directly above a line
    # saying APEX, which is unreadable and looks like the code is broken.
    def _says(v):
        if v == 0:
            return "nothing"
...
def draw_scene(img, r, headline, color, sub, fps):
    if not DRAW_OVERLAY:
        return                        # clean frame, for saving and analysing
    ...
    # --- the answer, drawn ON THE STOPPER END ---
    # The marker used to jump to whichever end won the vote, which made it look
    # like the code was "detecting the wrong side". It is not - the question
    # being answered is always "what is at the stopper", so the marker now stays
    # on the stopper end (band 0) and only the LABEL changes: green STEM when a
    # stem is at the stopper, red APEX when an apex is.
    if r["reason"] == "ok" and r["lo"] >= 0:
        ...
        # place the label just outside the marker, away from the chute
        cx, cy, cw, ch = CHANNEL_ROI
...
stalklost_warned = False # ditto for "a far-end stalk had to be discarded"
...
fitted = not FIT_ROI      # measure the chute once, on the first real frame
        ...
        if not present:
            ...
            if hold_n >= need or timed_out:
                ...
                if hold_n >= need:
                    ...
                    final_sure = False
                # A stalk was found at the far end and had to be discarded
                # because it left the box. That is the strongest cue there is,
                # thrown away - the answer cannot be called confident.
                if r["stalk_lost"]:
                    final_sure = False
                    if not stalklost_warned:
                        stalklost_warned = True
                        print("!!! a stalk at the FAR end ran off the edge of the")
                        print("    channel, so it could not be told from background")
                        print("    and was discarded. The best cue is being thrown")
                        print("    away. CHANNEL_ROI is too short for this chilli -")
                        print("    the pod must fit inside it with clear chute to")
                        print("    spare at the far end.")
                final_score = avg
                set_outputs(final)
                total += 1
                ...
                # `score` is the SMOOTHED average over SMOOTH_N frames, while
                # the cues beside it are this frame's. They can disagree, and
                # when they do it means a cue is flickering between frames -
                # worth knowing, so both numbers are shown rather than one.
